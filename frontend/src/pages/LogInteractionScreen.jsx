...
export default function LogInteractionScreen() {
  ...
  // Searchable HCP Dropdown states (Form mode)
  const [hcpSearchText, setHcpSearchText] = useState('');
  const [isHcpDropdownOpen, setIsHcpDropdownOpen] = useState(false);
  ...
  // Synchronize hcpSearchText when HCP selection changes
  useEffect(() => {
    if (currentFormState.hcp_id) {
      const selectedHcp = hcps.find(h => h.id.toString() === currentFormState.hcp_id.toString());
      if (selectedHcp) {
        setHcpSearchText(`${selectedHcp.name} (${selectedHcp.specialty})`);
      }
    } else {
      setHcpSearchText('');
    }
  }, [currentFormState.hcp_id, hcps]);
  ...
  const handleFormChange = (fields) => {
    dispatch(updateFormState(fields));
  };

  const handleAddFormSample = () => {
    const availableProduct = products.find(p => !currentFormState.samples.some(s => s.product_id === p.id));
    if (!availableProduct) return;
    
    handleFormChange({
      samples: [...currentFormState.samples, { product_id: availableProduct.id, quantity: 1 }]
    });
  };

  const handleRemoveFormSample = (index) => {
    const updated = currentFormState.samples.filter((_, i) => i !== index);
    handleFormChange({ samples: updated });
  };

  const handleFormSampleChange = (index, field, value) => {
    const updated = currentFormState.samples.map((s, i) => {
      if (i === index) {
  ...
  const handleProductToggle = (productId) => {
    const current = currentFormState.product_ids;
    if (current.includes(productId)) {
      handleFormChange({ product_ids: current.filter(id => id !== productId) });
    } else {
      handleFormChange({ product_ids: [...current, productId] });
    }
  };
  ...
  const handleSaveForm = (e) => {
    ...
    const payload = {
      hcp_id: parseInt(currentFormState.hcp_id),
      type: currentFormState.type,
      datetime: currentFormState.datetime ? new Date(currentFormState.datetime).toISOString() : new Date().toISOString(),
      discussion_notes: currentFormState.discussion_notes,
      sentiment: currentFormState.sentiment,
      follow_up_required: currentFormState.follow_up_required,
      follow_up_date: currentFormState.follow_up_required && currentFormState.follow_up_date ? currentFormState.follow_up_date : null,
      follow_up_notes: currentFormState.follow_up_required ? currentFormState.follow_up_notes : null,
      product_ids: currentFormState.product_ids.map(Number),
      samples: currentFormState.samples.map(s => ({
        product_id: parseInt(s.product_id),
        quantity: parseInt(s.quantity)
      }))
    };

    dispatch(saveInteraction({ data: payload, source: 'manual' })).then((res) => {
      if (!res.error) {
  ...
  const handleConfirmPreview = (previewCard) => {
    if (!previewCard.hcp_id) {
      alert("HCP must be resolved before confirmation.");
      return;
    }

    dispatch(confirmChatDraft({ previewCard, sessionId: 'default' })).then((res) => {
      if (!res.error) {
        alert("Interaction confirmed and logged successfully!");
        dispatch(setView('history'));
      }
    });
  };
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            {activeView === 'log' && (
              <div className="mode-toggle-container">
                ...
                <div
                  className={`mode-tab ${activeTab === 'form' ? 'active' : ''}`}
                  onClick={() => dispatch(setTab('form'))}
                >
                  Structured Form
                </div>
              </div>
        ...
        <div className="pane-container">
          {activeView === 'log' ? (
            activeTab === 'chat' ? (
                                      ...
                                      {hcps
                                        ...
                                        .map(h => (
                                          <div
                                            ...
                                            style={{ padding: '8px 12px', cursor: 'pointer', borderBottom: '1px solid rgba(224, 225, 247, 0.04)' }}
              ...
              /* Structured Form Mode View */
              <form className="form-pane" onSubmit={handleSaveForm}>
                <div className="form-grid">
                  {/* Searchable HCP Dropdown */}
                  <div className="form-group" style={{ position: 'relative' }} ref={dropdownRef}>
                    <label>Healthcare Professional (HCP) *</label>
                    <input
                      type="text"
                      placeholder="Search HCP by name or specialty..."
                      value={hcpSearchText}
                      onChange={(e) => {
                        setHcpSearchText(e.target.value);
                        setIsHcpDropdownOpen(true);
                      }}
                      onFocus={() => setIsHcpDropdownOpen(true)}
                      required={!currentFormState.hcp_id}
                    />
                    {isHcpDropdownOpen && (
                      <div
                        style={{
                      ...
                      >
                        {hcps
                          .filter(h => 
                            h.name.toLowerCase().includes(hcpSearchText.toLowerCase()) ||
                            h.specialty.toLowerCase().includes(hcpSearchText.toLowerCase())
                          )
                          .map(h => (
                            <div
                              key={h.id}
                              onClick={() => {
                                handleFormChange({ hcp_id: h.id.toString() });
                                setHcpSearchText(`${h.name} (${h.specialty})`);
                                setIsHcpDropdownOpen(false);
                              }}
                              style={{
                                padding: '10px 14px',
                                cursor: 'pointer',
                                borderBottom: '1px solid rgba(224, 225, 247, 0.04)',
                                transition: 'var(--transition-smooth)'
                              }}
                            ...
                            >
                              ...
                              <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{h.specialty} — {h.address}</div>
                            </div>
                          ))}
                        {hcps.filter(h => 
                          h.name.toLowerCase().includes(hcpSearchText.toLowerCase()) ||
                  ...
                  <div className="form-group">
                    ...
                    <input
                      type="datetime-local"
                      value={currentFormState.datetime}
                  ...
                  <div className="form-group">
                    ...
                    <select
                      value={currentFormState.sentiment}
                      onChange={(e) => handleFormChange({ sentiment: e.target.value })}
                    >
                      <option value="Positive">Positive</option>
                      <option value="Neutral">Neutral</option>
                      <option value="Negative">Negative</option>
                  ...
                  <div className="form-group full-width">
                    ...
                    <div className="product-multi-select">
                      {products.map(p => {
                        const isSelected = currentFormState.product_ids.includes(p.id);
                        return (
                          <span
                            key={p.id}
                            className={`product-option-pill ${isSelected ? 'selected' : ''}`}
                  ...
                  <div className="form-group full-width">
                    ...
                    <div className="samples-distributor">
                      {currentFormState.samples.map((sample, idx) => (
                        <div key={idx} className="sample-row">
                          <select
                            value={sample.product_id}
                            onChange={(e) => handleFormSampleChange(idx, 'product_id', e.target.value)}
                          >
                            {products.map(p => (
                              <option key={p.id} value={p.id}>{p.name}</option>
                            ))}
                          ...
                          <input
                            ...
                            onChange={(e) => handleFormSampleChange(idx, 'quantity', e.target.value)}
                          />
                          <button
                            type="button"
                            className="btn btn-secondary"
                            onClick={() => handleRemoveFormSample(idx)}
                            style={{ padding: '12px' }}
                          >
                            <Trash2 size={14} />
                          </button>
                  ...
                  <div className="form-group full-width">
                    ...
                    <textarea
                      placeholder="Add key details of the conversation..."
                      value={currentFormState.discussion_notes}
                      onChange={(e) => handleFormChange({ discussion_notes: e.target.value })}
                    />
                  </div>
                </div>
                ...
                <div className="followup-card">
                  <div
                    ...
                    onClick={() => handleFormChange({ follow_up_required: !currentFormState.follow_up_required })}
                  >
                    <input
                      type="checkbox"
                      checked={currentFormState.follow_up_required}
                      onChange={() => {}}
                    />
                    <label style={{ cursor: 'pointer' }}>Follow-up Required</label>
                  </div>

                  {currentFormState.follow_up_required && (
                    <div className="form-grid" style={{ marginBottom: 0 }}>
                      <div className="form-group">
                        <label>Follow-up Date</label>
                        <input
                          type="date"
                          value={currentFormState.follow_up_date}
                      ...
                      <div className="form-group">
                        ...
                        <input
                          type="text"
                          placeholder="e.g. Bring clinical study reports"
                          value={currentFormState.follow_up_notes}
